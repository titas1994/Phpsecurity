...
function createuser(){ ////use for create user in create file
	...
	/****** ----------------- *******/
	if(strlen($username) < $minmum)
	{
		echo "user name has longer to than 3";
	}
	elseif(strlen($username) > $maximum){ //username length check 

		echo "user name has longer to than 6";
	}
	elseif(in_array($username,$nam)){ //username validation
		echo "user name already exit";
	}
	elseif (strlen($password) < $minmum) {

		echo "password has longer than 3";
	}
	else{
